...
The remainder of this paper is organized as follows: Section 2 briefly reviews the theoretical foundations of GPs and DGPs, elucidating the applicability of multi-output DGPs in handling high-dimensional non-stationary responses. Section 3 details the proposed DA-DGP framework, focusing on the construction of the sample-level attention for precisely locating the ROI and the task-level attention mechanism for balancing multi-objective conflicts; furthermore, the weighted ELBO and the multi-objective robust optimization model are derived. Section 4 \textcolor{red}{presents numerical and engineering validation, including strong GP-specific baselines, ablation studies, sensitivity analyses, and robust Pareto/BO comparisons}. Finally, Section 5 concludes the paper and outlines future research directions.
...
\end{equation}
To capture the non-linear correlations with different sensitivities across input dimensions, \textcolor{red}{the current implementation of both case studies and all GP-family baselines employs the anisotropic Mat\'ern-5/2 kernel with Automatic Relevance Determination (ARD):}

为捕捉输入维度间具有不同敏感性的非线性相关关系，\textcolor{red}{当前两个案例及全部 GP 家族基线的实现均采用带有 Automatic Relevance Determination（ARD）的各向异性 Mat\'ern-5/2 核：}
\begin{equation}
...
\textbf{Evaluation Metrics and Baselines.} To assess both \textcolor{red}{local predictive fidelity near the target region} and \textcolor{red}{probabilistic calibration}, three metrics are reported on the \textcolor{red}{ROI-filtered test subset}: Root Mean Square Error (RMSE), Negative Log Predictive Density (NLPD), and Quality Loss (QL). The ROI is defined by \textcolor{red}{$|y_k-T_k|\leq 0.3$ for $k=1,2,3$}, so the evaluation focuses on \textcolor{red}{target-relevant local performance rather than global test-domain averages}. The comparison includes \textcolor{red}{ten methods}: the proposed \textcolor{red}{DA-DGP}; the optimization baselines \textcolor{red}{DWA, UW, and MGDA \citep{Liu2019,Kendall2018,Sener2018}}; the ablation chain \textcolor{red}{T-Atten, S-Atten (Equal), and Pure DGP}; and the strong GP-family baselines \textcolor{red}{LMC-DGP, Indep-DGP, and Indep-HetGP}. Here, \textcolor{red}{T-Atten keeps only task-level dynamic weighting}, \textcolor{red}{S-Atten (Equal) keeps sample-level attention but replaces meta-learned task weights with uniform coefficients}, and \textcolor{red}{Pure DGP removes both attention modules}.

\textbf{评估指标与基线方法。} 为同时评估\textcolor{red}{目标区域附近的局部预测精度}与\textcolor{red}{概率校准能力}，本文在\textcolor{red}{经过 ROI 筛选的测试子集}上报告三个指标：Root Mean Square Error（RMSE）、Negative Log Predictive Density（NLPD）和 Quality Loss（QL）。其中，ROI 定义为\textcolor{red}{$|y_k-T_k|\leq 0.3,\ k=1,2,3$}，因此这里衡量的是\textcolor{red}{与目标相关的局部性能，而不是全测试域平均表现}。对比对象共\textcolor{red}{10 种方法}：所提\textcolor{red}{DA-DGP}；优化基线\textcolor{red}{DWA、UW 和 MGDA \citep{Liu2019,Kendall2018,Sener2018}}；消融链\textcolor{red}{T-Atten、S-Atten（Equal）和 Pure DGP}；以及强 GP 家族基线\textcolor{red}{LMC-DGP、Indep-DGP 和 Indep-HetGP}。其中，\textcolor{red}{T-Atten 仅保留任务级动态加权}，\textcolor{red}{S-Atten（Equal）保留样本级注意力但将元学习任务权重替换为均匀系数}，\textcolor{red}{Pure DGP 则同时移除两类注意力模块}。
